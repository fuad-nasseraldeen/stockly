...
- ✅ **שדות מוצר מתקדמים** - SKU (מק"ט/ברקוד), כמות יחידות באריזה
...
- ✅ **הנחות מספקים** - אחוז הנחה לכל מחיר, חישוב אוטומטי של מחיר לאחר הנחה
- ✅ **כמות יחידות באריזה** - תמיכה בכמות יחידות באריזה (תלוי בספק)
- ✅ **חישוב מחיר מכירה אוטומטי** - עלות + הנחה + רווח + מע״מ (כולל הצגת *מחיר לפני מע״מ*)
- ✅ **הגדרות מחירים גמישות** - אפשרות להפעיל/לכבות שימוש במרווח ומע״מ
...
- ✅ **Bootstrap Endpoint** - טעינה אופטימלית של כל הנתונים הנדרשים בקריאה אחת
- ✅ **התאמה אישית של טבלאות** - שמירת פריסת עמודות מותאמת אישית למשתמש
- ✅ **העדפות משתמש** - שמירת העדפות משתמש (כמו פריסת טבלאות) פר חנות
...
2. הפעל את ה-extension הנדרש:
   ...
   ```
3. הרץ את המיגרציות בסדר הבא (לפי המספרים):
   - `0001_schema.sql` - סכמה בסיסית
   - `0002_rls_policies.sql` - RLS policies
   - `0003_migrate_existing_data.sql` - מיגרציה של נתונים קיימים (אם נדרש)
   - `0004_backfill_profiles.sql` - מילוי profiles (אם נדרש)
   - `0005_global_margin.sql` - מרווח גלובלי
   - `0006_Delete_reset_allDataBase.sql` - איפוס נתונים (אם נדרש)
...
> - מיגרציות 0003, 0004, 0006 הן אופציונליות (תלוי אם יש נתונים קיימים)
...
│   │   │   ├── bootstrap.ts   # API route לטעינה אופטימלית של נתונים
...
- **`suppliers`** - ספקים
- **`products`** - מוצרים (כולל `name_norm` לחיפוש, `sku`, `package_quantity`)
- **`price_entries`** - היסטוריית מחירים (כל שינוי מחיר יוצר רשומה חדשה, כולל `discount_percent`, `cost_price_after_discount`, `package_quantity`)
- **`settings`** - הגדרות מערכת פר חנות (מע״מ גלובלי, מרווח גלובלי, `use_margin`, `use_vat`)
- **`user_preferences`** - העדפות משתמש (פריסת טבלאות וכו') פר משתמש וחנות
...
POST   /api/products/:id/prices   # הוספת מחיר חדש למוצר (כולל הנחות וכמות יחידות)
GET    /api/products/:id/price-history?supplier_id=  # היסטוריית מחירים

GET    /api/bootstrap             # טעינה אופטימלית של כל הנתונים (tenants, settings, suppliers, categories, table layout)
...
### Supabase Pro Backups (מומלץ!)
- **גיבויים אוטומטיים:** Supabase Pro מספק גיבויים אוטומטיים כל יום
- **שמירה:** 7 ימים אוטומטית
- **שחזור:** דרך Supabase Dashboard - כמה קליקים
- **Point-in-Time Recovery:** ניתן לשחזר נתונים לנקודת זמן ספציפית
- **איך:** Supabase Dashboard → Database → Backups
- **יתרון:** לא צריך הגדרה - הכל אוטומטי!
