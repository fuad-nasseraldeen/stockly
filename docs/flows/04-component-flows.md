...
## 7) External Inventory

- `ניהול מלאי חיצוני` הוא מודול עצמאי ואינו משנה את מלאי הספקים, התראות מלאי, מחירים או מוצרים.
- בדאשבורד מוצג מספר המוצרים שנמצאים במלאי חיצוני מתוך כלל המוצרים.
- בדף הייעודי בוחרים מוצר ומעדכנים כמות בצעדים של `+1` או `-1`. מוצר שכמותו יורדת לאפס מוסר מהרשימה בלבד.
